codeunit 89953 "BAASIT Fleetrock E2E Tests"
{
    ...
    begin
        // [SCENARIO] With auto-posting off, the import leaves the invoice for someone to post.
        ImportPostAndSettleRepairOrder(false);
    end;

    [Test]
    procedure InvoicedRepairOrderIsAutoPostedOnImportAndDeducted()
    begin
        // [SCENARIO] With auto-posting on, the import posts the invoice itself, and posting it that
        // way raises the deduction just the same.
        ImportPostAndSettleRepairOrder(true);
    end;
    ...
    begin
        ...
        // auto-posting set for this scenario. The company's own setting is put back straight after
        // the import, the only step that reads it.
        JobQueueEntry.Init();
        ...
        OriginalAutoPost := SetAutoPostRepairOrders(AutoPostRepairOrders);
        if AutoPostRepairOrders then
        ...
        if AutoPostRepairOrders then
            InvoiceNo := AssertImportedAndAutoPosted(ROId)
        else
            InvoiceNo := AssertImportedAndPostByHand(ROId, ROObj);
    ...
    /// <summary>
    /// The import has left an unposted sales invoice for the repair order. Checks it against the
    /// repair order and posts it by hand, the way a user would, and hands back its number.
    /// </summary>
    local procedure AssertImportedAndPostByHand(ROId: Text; var ROObj: JsonObject) InvoiceNo: Code[20]
    var
    ...
    /// <summary>
    /// The import has posted the invoice itself. Checks nothing unposted is left behind, checks the
    /// posted invoice against the repair order, and hands back the number the invoice had before it
    /// was posted, which is the one the deduction records.
    /// </summary>
    local procedure AssertImportedAndAutoPosted(ROId: Text): Code[20]
    var
        DimSetEntry: Record "Dimension Set Entry";
        RepairHeaderStaging: Record "FRI Repair Header";
        SalesHeader: Record "Sales Header";
    ...
    begin
        // [THEN] The import posted the invoice, leaving no unposted invoice behind
        SalesHeader.SetRange("Document Type", SalesHeader."Document Type"::Invoice);
        SalesHeader.SetRange("FRI Fleetrock Repair Order No.", ROId);
        ...
        Assert.AreNotEqual('', SalesInvHeader."Pre-Assigned No.", 'The posted invoice should record the invoice it was posted from.');
        Assert.AreEqual(ROId, Format(SalesInvHeader."External Document No."), 'The repair order id should be carried as the external document number when the order has no PO number.');

        RepairHeaderStaging.SetRange(id, ROId);
        Assert.IsTrue(RepairHeaderStaging.FindLast(), 'A staging record should exist for the imported repair order.');
        Assert.IsTrue(RepairHeaderStaging."Invoiced At" <> 0DT, 'The staging record should carry the invoiced datetime.');
        Assert.AreEqual(DT2Date(RepairHeaderStaging."Invoiced At"), SalesInvHeader."Posting Date", 'The posting date should be the date the repair order was invoiced in Fleetrock.');
        ...
        Assert.AreEqual(2, SalesInvLine.Count(), 'The posted invoice should have exactly one labor line and one part line.');
        SalesInvLine.SetRange("No.", FleetrockSetup."Labor G/L Account No.");
        Assert.IsTrue(SalesInvLine.FindFirst(), 'The posted invoice should have a labor line.');
        Assert.AreEqual(150.0, SalesInvLine."Line Amount", 'The labor line amount should be the labor subtotal.');
        SalesInvLine.SetRange("No.", FleetrockSetup."Parts G/L Account No.");
        Assert.IsTrue(SalesInvLine.FindFirst(), 'The posted invoice should have a part line.');
        Assert.AreEqual(55.0, SalesInvLine."Line Amount", 'The part line amount should be the part subtotal.');
